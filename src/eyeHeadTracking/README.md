...
- **Architecture note**: The currently wired UI `experimental` eye/head mode still runs through the scheduler-based `EyeHeadTrackingService` stack. The newer `gaze/` module is present in the codebase, but it is not yet the main UI execution path. See [ARCHITECTURE_COMPARISON.md](ARCHITECTURE_COMPARISON.md) for the code-backed comparison.
