...
import type { BlinkStateStore } from './state';
...
export class BlinkScheduler {
  ...
  public triggerBlink(overrideIntensity?: number, overrideDuration?: number): void {
    ...
    try {
      ...
      // Schedule to animation service with autoPlay so it plays immediately
      const scheduledName = this.host.scheduleSnippet(snippet, { autoPlay: true });

      if (scheduledName) {
        // Record blink in state store
        this.store.recordBlink();
  ...
  private buildBlinkCurves(
    ...
    // Apply randomness to intensity (keep in 0-1 range for morph targets)
    const randomFactor = 1 + (Math.random() - 0.5) * this.config.randomness * 0.3;
    const finalIntensity = Math.min(1, intensity * randomFactor);
