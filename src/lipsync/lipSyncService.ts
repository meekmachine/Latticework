...
import { LipSyncScheduler, buildWordLipSyncSnippetName } from './lipSyncScheduler';
import type { LipSyncConfig, LipSyncCallbacks, VisemeEvent, AzureVisemeEvent } from './types';
...
export interface LipSyncServiceAPI {
  ...
  dispose: () => void;
  processAzureVisemes?: (events: AzureVisemeEvent[], totalDurationMs?: number) => string | null;
}
 ...
 * Tuned for faster Web Speech API TTS timing
 */
const DEFAULT_CONFIG: Required<LipSyncConfig> = {
  lipsyncIntensity: 1.0,
  speechRate: 1.0,
  onsetIntensity: 90,
  holdMs: 80, // Reduced from 140 for tighter timing
  engine: 'webSpeech',
  jawScale: 1.0,
};
...
export function createLipSyncService(
  ...
  return {
     ...
     */
    processWord(word: string, wordIndex: number): void {
      const snippetName = buildWordLipSyncSnippetName(word);
      machine.send({ type: 'PROCESS_WORD', word, wordIndex, snippetName });
      scheduler.processWord(word, wordIndex, undefined, snippetName);
    },
    ...
    processAzureVisemes(events: AzureVisemeEvent[], totalDurationMs?: number): string | null {
      return scheduler.processAzureVisemes(events, totalDurationMs);
    },
...
export class LipSyncService {
  ...
  /**
   * Process Azure viseme events from external audio playback
   */
  public processAzureVisemes(events: AzureVisemeEvent[], totalDurationMs?: number): string | null {
    return this.api.processAzureVisemes?.(events, totalDurationMs) ?? null;
  }
