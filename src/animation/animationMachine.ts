...
function coerceSnippet(d: NonNullable<LoadAnimationEvent['data']>, playing: boolean): NormalizedSnippet {
  ...
  const mixerLoopMode = (d as any).mixerLoopMode ?? (d.loop ? 'repeat' : 'once');
  ...
  return {
    ...
    aiExpressionMetadata: (d as any).aiExpressionMetadata ?? undefined,
    ...
    snippetEasing: (d as any).snippetEasing ?? 'linear',
    mixerLoopMode,
    mixerRepeatCount: typeof (d as any).mixerRepeatCount === 'number' ? (d as any).mixerRepeatCount : undefined,
    currentTime: 0,
