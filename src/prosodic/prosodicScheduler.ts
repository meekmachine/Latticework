...
export class ProsodicScheduler {
  ...
  private scheduleSnippet(snippet: ProsodicSnippet): void {
    ...
    // Use loop: false + mixerClampWhenFinished: true to match other agencies
    // The pulse system restarts animations on word boundaries
    const animSnippet = {
      name: snippet.name,
      curves: snippet.curves,
      maxTime: snippet.duration,
      loop: false,
      ...
      snippetBlendMode: 'additive' as const,
    };
