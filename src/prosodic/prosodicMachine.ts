...
function normalizeSnippet(data: any, category: 'brow' | 'head', priority: number): ProsodicSnippet | null {
  ...
  // Normalize intensity scale (some stored snippets use 0-100 instead of 0-1)
  let maxAbs = 0;
  Object.values(curves).forEach(arr => {
    arr.forEach(({ intensity }) => {
      const absVal = Math.abs(intensity ?? 0);
  ...
  if (maxAbs > 1) {
    Object.values(curves).forEach(arr => {
      arr.forEach((point) => {
        point.intensity = (point.intensity ?? 0) / 100;
      });
    });
  }
  ...
  return {
    ...
    category,
    priority,
    intensityScale: data.snippetIntensityScale ?? 0.7,
    isPlaying: false,
    loop: false, // Play once per pulse, scheduler restarts on word boundaries
    currentTime: 0,
    startWallTime: now,
    duration,
  ...
  actions: {
    ...
    activateBothChannels: assign(({ context }) => {
      ...
      const browSnippet = context.browSnippet ? {
        ...
        currentTime: 0,
        startWallTime: now,
        intensityScale: context.browSnippet.intensityScale,
      } : null;
      ...
      const headSnippet = context.headSnippet ? {
        ...
        currentTime: 0,
        startWallTime: now,
        intensityScale: context.headSnippet.intensityScale,
      } : null;
